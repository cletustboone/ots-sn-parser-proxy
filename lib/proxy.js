...
Proxy.prototype._transform = function( chunk, encoding, done ) {
  ...
  // Not a play by play file or MLB pitch file. Figure out sport.
    ...
    if ( parserType == "NO_PARSE" ) {
      ...
      this._exec( this.execParserCommand(parserType), done );
    }
...
Proxy.prototype._exec = function( cmd, done ) {

  // If the parser path is not defined, we just want to see what the command is
  if ( this.options["parser-path"] ) {
    // Execute the parser
...
Proxy.prototype.execParserCommand = function( type ) {
  ...
  command = "node",
  args = [
  (this.options["parser-path"] || "{{parser_path}}") + "/parsers/" + type + ".js",
    "--file",
    this.fileName,
    "--sport",
    this.fileSport
